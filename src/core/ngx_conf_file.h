...
struct ngx_command_s {
    ...
    //(*set)主要是从配置文件中把该指令的参数（存放在ngx_conf_t中）
    //转换为合适的数据类型并将转换后的值保存到模块的配置
    //结构体中（void *conf）
    char               *(*set)(ngx_conf_t *cf, ngx_command_t *cmd, void *conf); //数据存储函数
    ngx_uint_t            conf;         //数据存放在哪个配置
    ngx_uint_t            offset;       //数据存放在哪个位置
...
struct ngx_module_s {
    ...
    void                 *ctx;          //模块上下文，每个模块有不同模块上下文
    ngx_command_t        *commands;     //模块命令集
    ngx_uint_t            type;         //模块类型（HTTP, Core, Mail，Event中的一种）
