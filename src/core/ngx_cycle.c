...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    //创建所有模块配置的指针, 
    // 这里的ngx_max_module在nginx.c中计算过了
    //conf_ctx是所有模块的配置结构的指针数组
    ...
    //调用核心模块的配置创建函数， 
    //cycle->conf_ctx 中对应的指针指向创建的配置 
    //创建所有core module的configure.它通过调用每个
    //core module的ngx_xxx_module_create_conf方法，来创建对应的conf，
    //然后将这个conf对象保存在全局的conf_ctx中 
    for (i = 0; ngx_modules[i]; i++) {
        if (ngx_modules[i]->type != NGX_CORE_MODULE) {
            continue; 
			//这里只对核心模块进行处理，
			//核心模块就是ngx_core_module，ngx_errlog_module，
			//ngx_events_module和ngx_http_module，实际上只有ngx_core_module_create_conf
        }
    ...
    //开始解析配置文件了，解析配置文件它会一行行读取，
    //然后如果遇到指令
    //则会查找到对应的ngx_command_t对象，然后执行对应的回调set方法。
    //这里所有动作都在ngx_conf_parse这个函数中进行. 
    ...
    //打开所有的监听套接口（依次进行socket,bind,listen）
    ...
    //调用所有模块的ngx_XXX_module_init钩子，比如ngx_event_module_init
    for (i = 0; ngx_modules[i]; i++) {
